...
const RegisterForm = () => {
  ...
  useEffect(() => {
    ...
    // checck if device is mobile and redirect to mobile app
    if (window.innerWidth <= 768) {
      window.location.href = 'onodev-mobile://set-password/' + token
    } else {
      checkToken(token)
    }
  }, [isQueryReady])
  ...
  async function onSubmit(data: ISetPasswordFormData): Promise<void> {
    try {
      ...
      const result = await resetPassword({ ...data, resetPasswordToken: resetPasswordToken })
      if (!result) {
        showError(setPasswordFailedDescription)
      } else {
        const email: string = get(result, 'email', '')
        email !== '' && (await authStore.login({ email, password: newPassword }))
  ...
  if (isLoading) {
    return (
      <Stack>
        <Spinner thickness="4px" speed="0.65s" emptyColor="gray.200" color="blue.500" size="xl" alignSelf={'center'} />
      </Stack>
    )
  }
  ...
  return (
    <FormProvider {...method}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Stack spacing="6">
          <PasswordField name="newPassword" label="Your Password" placeholder="Your Password" />
          <PasswordField name="confirmNewPassword" label="Confirm Password" placeholder="Confirm Password" />
