...
export function getValidEnding(
  ...
  return `${
    amount < 1 && !isNoAmount
      ? nullString
      : `${isNoAmount ? "" : amount}${rightText ? "+" : ""} ${unitWithEnding}`
  }${rightText}`;
}
...
export function getCensorAddress(
  address: string,
  buildiumPropertyId: number
): string {
  const addressSplit: string[] = getValidArray<string>(
...
export function roundNumberToFixedDigits(
...
export function getRawUSPhoneNumber(phoneNumber: string): string {
  ...
  return rawPhoneNumber.startsWith(US_AREA_CODE)
    ? rawPhoneNumber
    : `${US_AREA_CODE}${rawPhoneNumber}`;
}
...
export function formatAvailableDate(availableDate: Date): string {
  const isBeforeOrSameNowDate: boolean = dayjs(availableDate).isSameOrBefore(
    dayjs()
  );
...
export function convertBreakPointToNumber(
  breakPoint: string,
  unit: string = "px"
): number {
